...
export default function FileSelector({
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFileItem &&
      setFileItem({
        fileType: config.acceptedFileTypes,
        fileName: file.name,
        file: file,
      });
  ...
  const handleSelection = async (item: string) => {
    ...
    try {
      ...
      setPending(true);
      setFileItem &&
        setFileItem({ fileType: config.acceptedFileTypes, fileName: item });
      ...
      const result = await toast.promise(
        promise,
        mergedConfig.selectionMessages!,
      );
  ...
  return (
    <div
      ref={dropdown}
      className={`relative z-10 ${firstUse ? "mt-4" : ""} ${className}`}
    >
      {/* Toggle button */}
      <button
        type="button"
      ...
      >
        ...
        <div className="flex justify-between">
          <p>
            {selected ||
              (firstUse
                ? mergedConfig.firstUseUploadText
                : mergedConfig.selectItemText)}
          </p>
          {firstUse ? (
            ""
          ) : (
      ...
      >
        ...
        />
        <label
          className="file_select_button cursor-pointer italic text-center animate-pulse"
          htmlFor={config.acceptedFileTypes}
        >
